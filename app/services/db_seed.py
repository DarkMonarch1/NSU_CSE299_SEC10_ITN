    ...
    try:
        ...
        if db.query(CompanyModel).count() == 0:
            ...
            try:
                ...
                logger.warning(f"Failed to seed companies: {e}")
        ...
        if db.query(JobPostingModel).count() == 0:
            ...
            try:
                ...
                logger.warning(f"Failed to seed job postings: {e}")
        ...
        # 5. Integrate Job Scraper for live job data (optional, skip if fails)
        if db.query(JobPostingModel).count() == 0:
            ...
            try:
                ...
                logger.warning(f"Job scraping integration skipped: {e}")
                
    except Exception as e:
        logger.error(f"Critical error during database seeding: {e}")
        # Don't raise - let the app start even if seeding fails
