...
def test_apply_job_endpoint(client) -> None:
    ...
    response = client.post(
        f"/jobs/{job_id}/apply",
        json={
            "applicantName": "Test Graduate",
            "applicantEmail": "test.grad@northsouth.edu",
            "resumeText": "Experienced Software Developer from NSU CSE.",
        },
    )
    assert response.status_code == 200
    data = response.json()
...
def test_auth_and_admin_endpoints(client) -> None:
    ...
    # Admin stats
    admin_res = client.get("/admin/stats")
