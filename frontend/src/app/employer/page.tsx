  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        console.error("Error loading employer portal data:", err);
  ...
  const handlePostJobSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const created = await createJob({
        ...
        requirements: ["B.S. in CSE from NSU", "Strong Problem Solving and Communication skills"],
        responsibilities: ["Build scalable backend APIs and user-friendly features"],
        benefits: ["Health Insurance", "Performance Bonus", "Hybrid flexibility"],
      });
      ...
      setJobFormError(err.message || "Failed to publish job. Please ensure you are logged in as an employer.");
    } finally {
      ...
      <ProtectedRoute>
        <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center py-12">
          <div className="rounded-3xl border border-pink-500/30 bg-slate-900/90 p-8 max-w-md text-center">
            ...
            <h2 className="text-xl font-bold text-white mb-2">Employer Role Required</h2>
            <p className="text-xs text-slate-300 mb-6">
              You must have an employer or corporate partner account to access the recruiter workspace and post jobs.
            </p>
            <button
          ...
          {showJobFormModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/85 p-4 backdrop-blur-md">
              <div className="w-full max-w-xl rounded-3xl border border-white/10 bg-slate-900 p-6 shadow-2xl">
                ...
                {jobFormError && (
                  <div className="mb-4 flex items-center gap-2 rounded-2xl border border-pink-500/30 bg-pink-500/10 p-3 text-xs font-semibold text-pink-400">
                    <AlertCircle className="h-4 w-4 shrink-0" />
                ...
                <form onSubmit={handlePostJobSubmit} className="space-y-4 text-xs text-slate-300">
                  ...
                  >
                    {isSubmitting ? "Persisting to Database..." : "Publish Job Posting"}
                  </button>
                </form>
              </div>
            </div>
